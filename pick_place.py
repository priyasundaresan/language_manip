...
class PickPlaceEnv():
  ...
  def __init__(self, BOUNDS, COLORS, PIXEL_SIZE):
    self.dt = 1/480
    self.sim_step = 0
  ...
  def get_observation(self):
    ...
    heightmap, colormap, xyzmap = self.get_heightmap(points, color, self.BOUNDS, self.PIXEL_SIZE)
  ...
  def get_heightmap(self, points, colors, bounds, pixel_size):
    ...
    xv, yv = np.meshgrid(np.linspace(self.BOUNDS[0, 0], self.BOUNDS[0, 1], height),
                         np.linspace(self.BOUNDS[1, 0], self.BOUNDS[1, 1], width))
    xyzmap[:, :, 0] = xv
    xyzmap[:, :, 1] = yv
